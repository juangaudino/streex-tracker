...
- Beta 0.10.0 Movement Capture & Zone Context has its migration applied to the active backend and its implementation pushed to `main`; deployment and authenticated owner QA remain unverified. Its approved interaction architecture is in `docs/MOVEMENT_CAPTURE_TECHNICAL_DESIGN.md`: Full Focus-first start/finish contextual capture, Entry as the same shared secondary control, Quick Actions as the only shift/accumulated-total hub, coarse private zones, no raw coordinate or route storage, and manual/Uber mileage remains authoritative. The next approved design block is Post-shift correction from Entry for explicitly selected finished shifts.
