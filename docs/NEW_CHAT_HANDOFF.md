...
- Current local source candidate: `Beta 0.9.10 - Route Bundle Performance (pending owner QA)`.
...
- The next release gate is a small-sample import, reload, cross-surface totals check, no-new-snapshots check, and gradual backfill review before publication; `Beta 0.9.11 - Deep Insights Productivity` follows after current correction and performance-release QA.
...
The expected next product item after the current correction and performance-release QA is the owner QA/publication gate for the local Historical Data Import (`0.9.7`), followed by Deep Insights Productivity (`0.9.11`). Bug stabilization or infrastructure work may still take priority and should renumber the roadmap honestly.
