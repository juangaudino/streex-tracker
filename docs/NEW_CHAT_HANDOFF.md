...
- Beta 0.10.1 Zone Intelligence & Evidence is approved for design only. It belongs as a third Deep Insights workspace, `Zone Intelligence`, with coverage-first reporting, private approximate zone cells, pickup-only eligible earnings, destination-flow context, and late-tip attribution. It must never infer zones, split batch money, expose raw location, or create a live navigation surface. Read `docs/ZONE_INTELLIGENCE_TECHNICAL_DESIGN.md` before implementation.
...
- Current real-work verification priority: `Beta 0.10.0 - Movement Capture & Zone Context` (implemented on `main`, active-backend migrations applied, initial authenticated iPhone QA complete). Preserve manual mileage and accumulated-total semantics; location remains foreground-only and never becomes a route tracker. The next approved design-only block is `Beta 0.10.1 - Zone Intelligence & Evidence`; do not implement it until its coverage-first contract is explicitly scheduled.
