...
export default function ManualRideAllocationLedger({ week, rides, snapshots, allocations, currencySymbol, onReplace }: {
  ...
  const items = useMemo(() => {
    ...
    return week.entries.flatMap((day) => Object.entries(day.apps)
      ...
      .filter(([app]) => !snapshotScopes.has(`${day.date}|${app}`))
  ...
  function beginEdit(item: typeof items[number]) {
    ...
    setDraftRows(item.rows.length ? item.rows.map(toDraft) : [{ key: crypto.randomUUID(), rideEventId: null, kind: "unassigned", amount: item.total, note: "Not yet assigned to a specific ride." }]);
  }
  ...
  return <section className="rounded-xl border border-primary/25 bg-primary/[0.03] p-3 space-y-3">
    <div>
      ...
      <p className="text-xs text-muted-foreground">Historical daily totals without Quick Actions snapshots. Confirm known ride amounts; leave the rest pending. This never changes the reported daily total.</p>
    </div>
    <div className="rounded-lg border border-primary/20 bg-background/70 p-2.5 text-xs text-muted-foreground"><span className="font-semibold text-foreground">Manual evidence:</span> the day total is fixed. You choose which recorded rides explain it, and the remaining amount stays visible until you know more.</div>
    {items.map((item) => {
      ...
      const rideChoices = rides.filter((ride) => ride.weekId === week.id && ride.dayDate === item.day.date && ride.app === item.app && ride.status !== "active");
      return <div key={key} className="rounded-lg border border-border bg-background/65 p-3 space-y-2">
        <div className="flex items-start justify-between gap-3">
          <div>
            <p className="text-xs font-semibold">{item.app} · {item.day.dayName} · reported {formatCurrency(item.total, currencySymbol)}</p>
            <p className="text-xs text-muted-foreground">Ride-assigned {formatCurrency(assigned, currencySymbol)} · Remaining {formatCurrency(remaining, currencySymbol)}</p>
          </div>
          {!editing && <Button type="button" size="sm" variant="outline" onClick={() => beginEdit(item)}>{item.rows.length ? "Review" : "Assign rides"}</Button>}
        </div>
        {!editing && item.rows.length === 0 && <p className="rounded-md border border-dashed border-border px-2.5 py-2 text-xs text-muted-foreground">No ride amount confirmed yet. {rideChoices.length ? "Assign the rides you know, then leave any remainder pending." : "Record already-counted rides in the relevant closed shift first; Streex will not invent them or change the day total."}</p>}
        {!editing ? item.rows.map((row) => {
          ...
          {draftRows.map((row) => <div key={row.key} className="grid grid-cols-[1fr_92px_auto] gap-2">
            <select className="h-10 min-w-0 rounded-md border border-input bg-background px-2 text-xs" value={row.rideEventId ?? "unassigned"} onChange={(event) => {
              ...
              {rideChoices.map((ride) => <option key={ride.id} value={ride.id}>{ride.app} · {new Date(ride.endedAt ?? ride.startedAt).toLocaleTimeString([], { hour: "numeric", minute: "2-digit" })}</option>)}
            </select>
