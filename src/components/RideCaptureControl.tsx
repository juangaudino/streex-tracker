...
export default function RideCaptureControl({ openWeek, apps, rideEvents, onStart, onFinish, onCancel, onUpdateTotals, compact = false }: RideCaptureControlProps) {
  ...
  async function startRide() {
    ...
    setLastCaptureResult(!event ? "Ride was not saved. Try again." : capture.status === "captured" ? "Start zone saved" : capture.status === "denied" ? "Location denied — ride started without zone" : "Start zone unavailable — ride started without zone");
    setCapturing(null);
  }
        ...
        <div className="min-w-0 flex-1">
          ...
          {activeRide ? (
            ...
            <>
              ...
              <p className="text-[11px] text-muted-foreground">{completedPending ? `${completedPending} completed ride${completedPending === 1 ? "" : "s"} pending · ${latestCompleted?.endCaptureStatus === "captured" ? "End zone saved" : latestCompleted?.endCaptureStatus === "denied" ? "Location denied" : "End zone unavailable"}` : hasShift ? "Capture ride zones while working" : "Start a shift in Quick Actions first"}</p>
            </>
          )}
        </div>
      </div>
