...
function Metric({ label, value }: { label: string; value: string }) {
  return <div className="rounded-2xl border border-white/10 bg-white/[0.05] p-5"><p className="text-[11px] font-black uppercase tracking-[0.2em] text-white/45">{label}</p><p className="mt-2 font-mono text-3xl font-black text-white">{value}</p></div>;
}

function CardShell({ page, title, subtitle, children }: { page: number; title: string; subtitle: string; children: React.ReactNode }) {
  return (
    <article className="relative h-[960px] w-[540px] shrink-0 overflow-hidden bg-[#080A09] p-10 text-white" style={{ backgroundImage: "radial-gradient(circle at 100% 0%, rgba(230,206,32,.22), transparent 30%), linear-gradient(160deg,#080A09,#111411 55%,#080A09)" }}>
      <div className="flex items-center justify-between"><span className="text-2xl font-black tracking-tight">STREE<span className="text-[#E6CE20]">X</span></span><span className="font-mono text-xs text-white/40">0{page}/03</span></div>
      <div className="mt-12"><p className="text-xs font-black uppercase tracking-[0.3em] text-[#E6CE20]">Driver Playbook</p><h2 className="mt-3 text-5xl font-black leading-[0.95] tracking-tight">{title}</h2><p className="mt-4 max-w-[430px] text-base leading-relaxed text-white/55">{subtitle}</p></div>
      <div className="mt-10">{children}</div>
      <div className="absolute bottom-9 left-10 right-10 border-t border-white/10 pt-5 text-[11px] leading-relaxed text-white/40">Personal observed patterns — not an earnings guarantee.</div>
    </article>
  );
}
  ...
  return <div className="fixed inset-0 z-[100] overflow-y-auto bg-black/80 p-4 backdrop-blur-md">
    <div className="mx-auto max-w-[1780px]">
      ...
      <div className="flex gap-5 overflow-x-auto pb-8">
        ...
        <div><div ref={(node) => { refs.current[1] = node; }}><CardShell page={2} title="Best times" subtitle="The strongest repeatable windows inside the selected operating scope."><div className="space-y-3">{op.bestWindows.slice(0, 4).map((row, index) => <div key={row.hour} className="flex items-center justify-between rounded-2xl border border-white/10 bg-white/[0.05] p-5"><div><p className="text-xs font-black uppercase tracking-widest text-[#E6CE20]">#{index + 1} window</p><p className="mt-1 text-2xl font-black">{row.label}</p><p className="mt-1 text-xs text-white/45">{row.hours.toFixed(1)} worked hours · {row.source}</p></div><p className="font-mono text-xl font-black">{row.earningsPerHour == null ? "—" : `${formatCurrency(row.earningsPerHour, data.currencySymbol)}/hr`}</p></div>)}</div><div className="mt-6 grid grid-cols-2 gap-3">{strongestDays.slice(0, 4).map((day) => <Metric key={day.dayName} label={`${day.dayName} · ${new Date(2020, 0, 1, day.hour).toLocaleTimeString("en-US", { hour: "numeric" })}`} value={day.rate == null ? "—" : `${formatCurrency(day.rate, data.currencySymbol)}/hr`} />)}</div></CardShell></div><button onClick={() => saveOne(1)} className="mt-2 w-full rounded-xl bg-white/10 py-2 text-sm font-bold text-white">Save card 2</button></div>
        <div><div ref={(node) => { refs.current[2] = node; }}><CardShell page={3} title="Operating playbook" subtitle="A compact field guide built from the patterns in this filtered view."><div className="space-y-4">{op.observations.map((observation, index) => <div key={observation} className="rounded-2xl border border-white/10 bg-white/[0.05] p-5"><p className="text-xs font-black uppercase tracking-[0.2em] text-[#E6CE20]">Signal 0{index + 1}</p><p className="mt-3 text-xl font-bold leading-snug">{observation}</p></div>)}</div><div className="mt-6 grid grid-cols-2 gap-3"><Metric label="Miles / ride" value={metric(op.totals.milesPerRide)} /><Metric label="Minutes / ride" value={metric(op.totals.minutesPerRide)} /><Metric label="Earnings / ride" value={op.totals.earningsPerRide == null ? "—" : formatCurrency(op.totals.earningsPerRide, data.currencySymbol)} /><Metric label="Observed coverage" value={`${op.coverage.toFixed(0)}%`} /></div></CardShell></div><button onClick={() => saveOne(2)} className="mt-2 w-full rounded-xl bg-white/10 py-2 text-sm font-bold text-white">Save card 3</button></div>
      </div>
    </div>
  </div>;
}
