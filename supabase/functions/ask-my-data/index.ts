...
function directMobilityAnswer(context: unknown, currency: string, prompt: string): string | null {
  const c = context as {
    analysis?: {
      mobility?: {
        pickupEarnings?: {
...
Deno.serve(async (req) => {
  ...
  if (needsMobility) {
    const text = directMobilityAnswer(context, settingsRes.data?.currency_symbol ?? "$", promptPreview);
    if (text) {
      await logUsage({
        supabase,
        userId,
